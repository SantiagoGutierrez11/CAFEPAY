...
DROP TABLE WORKER_PAYMENT_SEQ CASCADE CONSTRAINTS;
...
--------------------------------------------------------
-- TABLA AUXILIAR: Secuencia de PAYMENT por WORKER_CODE
--------------------------------------------------------
CREATE TABLE WORKER_PAYMENT_SEQ (
    WORKER_CODE VARCHAR2(6)   NOT NULL,
    NEXT_ID     NUMBER(10,0)  NOT NULL,
    CONSTRAINT WORKER_PAYMENT_SEQ_PK PRIMARY KEY (WORKER_CODE)
);

--------------------------------------------------------
-- TABLA PAYMENT (FACTURA/PAGO)
-- CLAVE PRIMARIA COMPUESTA: (WORKER_CODE, IDPAYMENT)
--------------------------------------------------------
CREATE TABLE PAYMENT (
    WORKER_CODE     VARCHAR2(6)   NOT NULL,   -- FK a COLLECTOR (parte de PK)
    IDPAYMENT       NUMBER(10,0)  NOT NULL,   -- ID secuencial por trabajador (parte de PK)
...
ALTER TABLE PAYMENT
  ADD CONSTRAINT PAYMENT_PK PRIMARY KEY (WORKER_CODE, IDPAYMENT) ENABLE;
...
-- FK a PAYMENT (llave compuesta)
ALTER TABLE COLLECT
  ADD CONSTRAINT COLLECT_FK_PAYMENT 
  FOREIGN KEY (WORKER_CODE, IDPAYMENT)
  REFERENCES PAYMENT (WORKER_CODE, IDPAYMENT) ENABLE;
...
--------------------------------------------------------
-- TRIGGER 1 PAYMENT: IDPAYMENT incremental por WORKER_CODE
--------------------------------------------------------
...
BEGIN
    IF :NEW.IDPAYMENT IS NULL THEN
        BEGIN
            -- Intentar obtener el siguiente ID para este trabajador
            SELECT NEXT_ID INTO v_next
              FROM WORKER_PAYMENT_SEQ
             WHERE WORKER_CODE = :NEW.WORKER_CODE
             FOR UPDATE;
            
            :NEW.IDPAYMENT := v_next;
            
            -- Incrementar para el próximo
            UPDATE WORKER_PAYMENT_SEQ
               SET NEXT_ID = v_next + 1
             WHERE WORKER_CODE = :NEW.WORKER_CODE;
             
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                -- Primera factura de este trabajador
                :NEW.IDPAYMENT := 1;
                INSERT INTO WORKER_PAYMENT_SEQ (WORKER_CODE, NEXT_ID)
                VALUES (:NEW.WORKER_CODE, 2);
        END;
    END IF;
END;
/
...
--------------------------------------------------------
-- TRIGGER 3 COLLECT: Validar solo un ZERO por asociación
--------------------------------------------------------
...
--------------------------------------------------------
-- TRIGGER 4 COLLECT: Calcular AMOUNT_TO_PAY automáticamente
--------------------------------------------------------
...
--------------------------------------------------------
-- TRIGGER 5 COLLECT: Validar que solo se facturen recolectas REGISTRADAS
--------------------------------------------------------
...
--------------------------------------------------------
-- TRIGGER 6 PAYMENT: Actualizar TOTAL_AMOUNT automáticamente
--------------------------------------------------------
...
-- El trigger COLLECT_BU_VALIDATE_PAYMENT cambiará STATUS_ID a 2 automáticamente
...
-- Crear PRIMERA factura para W00002 (IDPAYMENT será 1 para este trabajador)
INSERT INTO PAYMENT (IDPAYMENT, WORKER_CODE, PAYMENT_DATE, TOTAL_AMOUNT, STATUS_ID, NOTES)
...
-- Crear factura para W00003 (IDPAYMENT será 1 para este trabajador)
